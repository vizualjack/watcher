...
function showResult(seriesData) {
    seriesData.forEach((val, index) => {
        ...
        seriesEle.addEventListener('click', async () => {
            await window.backend.loadSeries(index);
...
async function search() {
    clearResult()
    showLoadAnimation();
    let seriesData = await window.backend.searchSeries(searchText.value);
...
async function load() {
    let seriesData = await window.backend.getSearchResult();
    showResult(seriesData);
}
